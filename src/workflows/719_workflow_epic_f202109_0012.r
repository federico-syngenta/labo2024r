...
HT_tuning_epic <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$bo_iteraciones <- 20 #ieraciones de la Optimizacion Bayesiana
...
wf_septiembre <- function( pnombrewf )
{
  ...
  CN_canaritos_asesinos_base(ratio=0.8, desvio=1.2)
  
  ts9 <- TS_strategy_base9()
  ht <- HT_tuning_epic()
  
  fm <- FM_final_models_lightgbm_semillerio( c(ht, ts9), ranks=c(1), semillerio=5,repeticiones_exp=5 )
