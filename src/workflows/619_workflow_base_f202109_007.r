...
#NUEVA CORRIDA DE BASE PARA CANARITOS ASESINOS
#Ratio=0.3; Desvio=3.35
#Ratio=0.55; Desvio=0.5
#Ratio=1.00; Desvio=1.5
#Ratio=0.4; Desvio=-2.0
...
# Definir la función wf_septiembre
wf_septiembre <- function(pnombrewf, ratio, desvio) {
  param_local <- exp_wf_init(pnombrewf) # línea fija
  ...
  CN_canaritos_asesinos_base(ratio = ratio, desvio = desvio)  # Valores dinámicos
  ...
  return(exp_wf_end()) # línea fija
}

# Valores a asignar
ratios <- c(0.9599, 0.2463, 0.293785, 4.0,0.05)
desvios <- c(2.332455, 0.7455,0.3214,0.05,0.5)

# Iterar sobre los valores y llamar a la función wf_septiembre con los nuevos valores
for (i in 1:length(ratios)) {
  cat("Iteración", i, "- Usando ratio:", ratios[i], "y desvio:", desvios[i], "\n")  # Imprimir los valores de ratio y desvio
  resultado <- wf_septiembre("nombre_wf", ratios[i], desvios[i])  # Llamar a la función con los nuevos valores
  # Realizar acciones con el resultado si es necesario
}
