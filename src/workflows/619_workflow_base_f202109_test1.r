...
FErf_attributes_base <- function( pinputexps, ratio, desvio)
{
  ...
  param_local$lgb_param <- list(
    ...
    num_iterations = 40, #cambiado antes era 20
    num_leaves  = 20, #cambiado antes era 16
    min_data_in_leaf = 1020, #cambiado antes era 1000
    feature_fraction_bynode  = 0.5, #cambiado antes era 0.2
...
HT_tuning_base <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$bo_iteraciones <- 65 # iteraciones de la Optimizacion Bayesiana
  #lo cambie antes era 60
  return( exp_correr_script( param_local ) ) # linea fija
}
#------------------------------------------------------------------------------
...
wf_septiembre <- function( pnombrewf )
{
  ...
  CN_canaritos_asesinos_base(ratio=0.8, desvio=1.0)#cambiado antes era 0.2 y 4.0
